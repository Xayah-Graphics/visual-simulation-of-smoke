...
void set_u_boundary(float* u, const int nx, const int ny, const int, const std::vector<int>& slices) {
...
void set_v_boundary(float* v, const int nx, const int ny, const int, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int i = 0; i < nx; ++i) {
...
void set_w_boundary(float* w, const int nx, const int ny, const int nz, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int j) {
        for (int i = 0; i < nx; ++i) {
...
void compute_vorticity(const float* u, const float* v, const float* w, float* omega_x, float* omega_y, float* omega_z, float* omega_mag, const int nx, const int ny, const int nz, const float h, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < nx; ++i) {
...
void compute_confinement(const float* omega_x, const float* omega_y, const float* omega_z, const float* omega_mag, float* force_x, float* force_y, float* force_z, const int nx, const int ny, const int nz, const float epsilon, const float h, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < nx; ++i) {
...
void apply_u_forces(float* u, const float* force_x, const int nx, const int ny, const int, const float dt, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 1; i < nx; ++i)
...
void apply_v_forces(float* v, const float* density, const float* temperature, const float* force_y, const int nx, const int ny, const int, const float ambient_temperature, const float density_buoyancy, const float temperature_buoyancy, const float dt, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 1; j < ny; ++j)
            for (int i = 0; i < nx; ++i) {
...
void apply_w_forces(float* w, const float* force_z, const int nx, const int ny, const int, const float dt, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < nx; ++i)
...
void advect_u(float* dst, const float* src_u, const float* src_v, const float* src_w, const int nx, const int ny, const int nz, const float h, const float dt, const bool cubic, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i <= nx; ++i) {
...
void advect_v(float* dst, const float* src_u, const float* src_v, const float* src_w, const int nx, const int ny, const int nz, const float h, const float dt, const bool cubic, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j <= ny; ++j)
            for (int i = 0; i < nx; ++i) {
...
void advect_w(float* dst, const float* src_u, const float* src_v, const float* src_w, const int nx, const int ny, const int nz, const float h, const float dt, const bool cubic, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < nx; ++i) {
                float px = (static_cast<float>(i) + 0.5f) * h;
...
void subtract_gradient_v(float* v, const float* pressure, const int nx, const int ny, const int, const float h, const float dt, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 1; j < ny; ++j)
            for (int i = 0; i < nx; ++i)
...
void subtract_gradient_w(float* w, const float* pressure, const int nx, const int ny, const int, const float h, const float dt, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < nx; ++i)
...
void advect_scalar(float* dst, const float* src, const float* u, const float* v, const float* w, const int nx, const int ny, const int nz, const float h, const float dt, const bool cubic, const bool clamp_nonnegative, const std::vector<int>& slices) {
    std::for_each(std::execution::par_unseq, slices.begin(), slices.end(), [&](const int k) {
        for (int j = 0; j < ny; ++j)
            for (int i = 0; i < nx; ++i) {
                float px = (static_cast<float>(i) + 0.5f) * h;
...
int32_t visual_simulation_of_smoke_step_parallel(const VisualSimulationOfSmokeStepDesc* desc) {
    ...
    auto* density = static_cast<float*>(desc->density);
    auto* temperature = static_cast<float*>(desc->temperature);
    ...
    auto* previous_density = static_cast<float*>(desc->temporary_previous_density);
    auto* previous_temperature = static_cast<float*>(desc->temporary_previous_temperature);
    auto* previous_velocity_x = static_cast<float*>(desc->temporary_previous_velocity_x);
    auto* previous_velocity_y = static_cast<float*>(desc->temporary_previous_velocity_y);
    auto* previous_velocity_z = static_cast<float*>(desc->temporary_previous_velocity_z);
    ...
    auto* omega_magnitude = static_cast<float*>(desc->temporary_omega_magnitude);
    ...
    std::vector<int> interior_w_slices(static_cast<std::size_t>(std::max(nz - 1, 0)));
    ...
    apply_u_forces(velocity_x, force_x, nx, ny, nz, dt, cell_slices);
    apply_v_forces(velocity_y, density, temperature, force_y, nx, ny, nz, ambient_temperature, density_buoyancy, temperature_buoyancy, dt, cell_slices);
    apply_w_forces(velocity_z, force_z, nx, ny, nz, dt, interior_w_slices);

    advect_u(previous_velocity_x, velocity_x, velocity_y, velocity_z, nx, ny, nz, cell_size, dt, cubic, cell_slices);
    advect_v(previous_velocity_y, velocity_x, velocity_y, velocity_z, nx, ny, nz, cell_size, dt, cubic, cell_slices);
    advect_w(previous_velocity_z, velocity_x, velocity_y, velocity_z, nx, ny, nz, cell_size, dt, cubic, w_slices);
    set_u_boundary(previous_velocity_x, nx, ny, nz, cell_slices);
    set_v_boundary(previous_velocity_y, nx, ny, nz, cell_slices);
    ...
    subtract_gradient_w(previous_velocity_z, pressure, nx, ny, nz, cell_size, dt, interior_w_slices);
    set_u_boundary(previous_velocity_x, nx, ny, nz, cell_slices);
    set_v_boundary(previous_velocity_y, nx, ny, nz, cell_slices);
    set_w_boundary(previous_velocity_z, nx, ny, nz, y_slices);
    ...
    advect_scalar(density, previous_density, velocity_x, velocity_y, velocity_z, nx, ny, nz, cell_size, dt, cubic, true, cell_slices);
    advect_scalar(temperature, previous_temperature, velocity_x, velocity_y, velocity_z, nx, ny, nz, cell_size, dt, cubic, false, cell_slices);
